    ...
    for (const gw of (data.value || [])) {
      try {
        ...
        (ds.value || []).forEach(src => result.push({
          ...
          username: src.credentials?.username || src.credentialDetails?.username || src.username || null
        }));
      } catch (e) { }
    }
    return result;
...
async function fetchConnections(token) {
  try {
    ...
    return (data.value || []).map(c => ({
      ...
      username: c.credentials?.username || c.credentialDetails?.username || c.username || null
    }));
  } catch (e) { return []; }
}
...
app.http('pbi-proxy', {
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (action === 'testDatasource') {
        const token = await getToken();
        const gateways = await pbiGet(token, '/gateways');
        const gw = (gateways.value || [])[0];
        if (!gw) return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ message: 'No gateways found' }) };
        const sources = await pbiGet(token, `/gateways/${gw.id}/datasources`);
        const src = (sources.value || [])[0];
        return { status: 200, headers: CORS_HEADERS, body: JSON.stringify({ credentials: src?.credentials, credentialType: src?.credentialType, fullSource: src }) };
      }
